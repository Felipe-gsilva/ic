...
\node[dashedbox, right=2.2cm of fold1, yshift=0.6cm] (preproc1) {a};
\node[dashedbox, below=of preproc1] (preproc2) {b};
...
% Training/Validation
\node[dashedbox, right=2cm of preproc] (train) {Conjunto\\de Treino};
\node[dashedbox, below=of train] (val) {Conjunto\\de Validação};
\node[box, fit=(train)(val), label=above:Conjuntos, inner sep=5pt] (conjuntos) {};

\draw[arrow] (preproc.east) -- (train.west);
\draw[arrow] (preproc.east) -- (val.west);

% GAN
\node[box, above=of preproc] (gan) {GAN};
\node[box, above=of train] (synthetics) {Imagens\\sintéticas};
